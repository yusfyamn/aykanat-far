...
export const PREMIUM_VIEWPORT_MARGIN = "0px 0px -26% 0px";
export const PREMIUM_VIEWPORT_AMOUNT = 0.22;
export const PREMIUM_VIEWPORT_MARGIN_SOFT = "0px 0px -18% 0px";
export const PREMIUM_VIEWPORT_AMOUNT_SOFT = 0.14;

const getMotionProfile = () => {
  if (typeof window === "undefined") return "desktop";
  const prefersReduced = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  if (prefersReduced) return "reduced";
  const isMobile = window.innerWidth < 1025 || window.matchMedia("(pointer: coarse)").matches;
...
export const premiumReveal = {
  initial: {
    opacity: 0,
    filter: "blur(18px) saturate(120%) brightness(1.04) contrast(0.98)",
  },
  animate: {
    opacity: 1,
